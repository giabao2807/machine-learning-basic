...
np.set_printoptions(precision=3, threshold=10, edgeitems=4,
                    linewidth=120)  # Prettier array printing
...
def simplex(A: matrix, b: np.array, c: np.array, rule: int = 0):
    ...
    if b.shape != (m,):
        raise ValueError(
            "Incompatible dimensions: c_j has shape {}, expected {}.".format(b.shape, (m,)))
    if c.shape != (n,):
        raise ValueError(
            "Incompatible dimensions: c has shape {}, expected {}.".format(c.shape, (n,)))

    "Check full rank matrix"
    if not np.linalg.matrix_rank(A) == m:
        # Remove ld rows:
        A = A[[i for i in range(m) if not np.array_equal(
            np.linalg.qr(A)[1][i, :], np.zeros(n))], :]
    ...
    # Phase I constraint matrix
    A_I = matrix(np.concatenate((A, np.identity(m)), axis=1))
    ...
    ext_I, x_init, basic_init, z_I, _, it_I = simplex_core(
        A_I, c_I, x_I, basic_I, rule)
    # ^ Exit code, initial BFS, basis, z_I, d (not needed) and no. of iterations
    print("Phase I terminated.")

    # assert that phase I has an optimal solution (and is not unlimited)
    assert ext_I == 0
    if z_I > 0:
        print("\n")
    ...
    # Get initial BFS for original problem (without artificial vars.)
    x_init = x_init[:n]
    ...
    if ext == 0:
        ...
        print_boxed(
            "Unlimited problem. Found feasible ray d =\n{}\nfrom x =\n{}.".format(d, x))
...
def simplex_core(A: matrix, c: np.array, x: np.array, basic: set, rule: int = 0):
    ...
    assert isinstance(basic, set) and len(basic) == m and \
        all(i in range(n)
            for i in basic)  # Make sure that basic is a valid base
    ...
    # Ensure procedure terminates (for the min reduced cost rule)
    while it <= 500:
        ...
        if len(neg) == 0:
            ...
            # Flag problem as unlimited and return ray
            return 1, x, set(B),  None, d, it

        # Get theta and index (in basis) of exiting basic variable:
        theta, p = min(neg, key=(lambda tup: tup[0]))

        """Variable updates"""
        x = np.array([trunc(var)
                     for var in (x + theta * d)])  # Update all variables
        assert x[B[p]] == 0
    ...
    raise TimeoutError(
        "Iterations maxed out (probably due to an endless loop)")
